...
def generate_imageset(dir, dst_path):
    files = os.listdir(dir)
    names = [osp.splitext(x)[0] for x in files]
...
#     work_dir = 'work_dirs/s2anet_r50_fpn_1x_ms_rotate'
    cfg = Config.fromfile(config_file)
    root_dir = osp.split(cfg.data['test'].ann_file)[0]

    generate_imageset(root_dir + '/labels', root_dir +'/imageset.txt')

    detpath = osp.join(work_dir, 'result_raw/Task1_{:s}.txt')
    annopath =  root_dir + '/labelTxt/{:s}.txt' # change the directory to the path of val/labelTxt, if you want to do evaluation on the valset
    
    imagesetfile = root_dir +'/imageset.txt'
