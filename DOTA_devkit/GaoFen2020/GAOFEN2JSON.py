...
def generate_json_labels(src_path, out_path, trainval=True, type='plane'):
    ...
    with open(out_path, 'w') as f:
        ...
        for id, img in enumerate(pbar):
            pbar.set_description("GaoFen2020 generate_json_labels")
            img_info = {}
            img_name = osp.splitext(img)[0]
            label = os.path.join(label_path, img_name+'.txt')
            img = Image.open(osp.join(img_path, img))
            img_info['filename'] = img_name + '.tif' if type == 'plane' else img_name + '.tiff'
