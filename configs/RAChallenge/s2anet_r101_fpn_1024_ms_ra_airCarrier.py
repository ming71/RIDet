...
train_pipeline = [
    ...
    dict(type='RotatedResize', img_scale=(1024, 1024), keep_ratio=True),  # 添加0.75尺度
...
dataset_type = 'RAChallengeAirCarrierDataset'
data_root = ' '
stage2_test_img_root = 'data/RAChallenge/stage2/stage2_air_carrier/'
stage2_test_json_root = 'data/RAChallenge/stage2/'
...
data = dict(
    ...
    test=dict(
        type=dataset_type,
        ann_file=stage2_test_json_root + 'test_air_carrier.json',
        img_prefix=stage2_test_img_root,
        pipeline=test_pipeline)
)

# optimizer
# optimizer = dict(type='SGD', lr=1e-4, momentum=0.9, weight_decay=0.0001)
optimizer = dict(type='Adam', lr=1e-4, betas=(0.9, 0.999), eps=1e-08, weight_decay=0)
...
log_level = 'INFO'
work_dir = 'work_dirs/s2anet_r101_fpn_1x_ms_ra_airCarrier/'
load_from = 'work_dirs/s2anet_r101_fpn_1x_ms_ra_airCarrier/epoch_60.pth'
resume_from = None
workflow = [('train', 1)]