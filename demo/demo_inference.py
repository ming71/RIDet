...
def save_det_result_bbox(config_file, out_dir, checkpoint_file=None, img_dir=None):
    ...
    for img_name in img_list:
        ...
        img_out_path = osp.join(out_dir, img_name)
        result = inference_detector(model, img_path)
        show_result(img_path, result, model.CLASSES, show=False, score_thr=0.5, out_file=img_out_path)
        print(img_out_path)
...
if __name__ == '__main__':
    ...
    ROOT_dir = args.config_file.split('/')[1]
    if ROOT_dir in ['HRSC2016', 'ICDAR2015', 'MSRA_TD500']:
        # colormap=[(212, 188, 0),]
        colormap=[(0, 255, 0),]
    elif ROOT_dir == 'DOTA':
        colormap = [
    ...
    save_det_result(args.config_file, 
                    args.out_dir,
